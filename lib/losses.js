...
// Enters under an exemption regime, not as a commercial import: military
// equipment and aircraft with their parts. No VAT is booked on entry.
export const EXEMPT_HS = ["8710", "8802", "8803", "8805", "8806", "8906", "93"];
export const isExempt = (code) => EXEMPT_HS.some((p) => code.startsWith(p));
...
export function summary(corridors) {
  ...
  return {
    ...
    tradeValue: add(corridors.filter((c) => c.signature !== "structural"), "x_cif"),
    declared: add(corridors.filter((c) => c.signature !== "structural"), "m"),
  };
}
